...
def write_config_file(config,
    ...
    if 'dagman_submits_interval' in config.keys():
        lines.append('DAGMAN_MAX_SUBMITS_PER_INTERVAL={}'.format(
    ...
    for i, script_i in enumerate(script_files):
        ...
        if config['merge_files']:
            ...
            log_dir = os.path.join(log_dir_base,
                                   '{0:04d}000-{0:04d}999'.format(i//1000))
            if not os.path.isdir(log_dir):
                os.makedirs(log_dir)

        job_name = '{}_{}'.format(process_name, i)
        lines.append('JOB {} {}'.format(job_name, job_file))
        lines.append('VARS {} script_file="{}" run="{}" log_dir="{}"'.format(
