...
inpath = os.path.join(PATHS.data, "check_ehe_mc_ids_bf")
outpath = os.path.join(PATHS.local, "check_ehe_mc_ids_bf")

path_names = ["check_ehe_alert_mc_ids_bf", "check_ehe_alert_hb_mc_ids_bf", "check_hese_mc_ids_bf"]


for item in path_names:
	inpath = os.path.join(PATHS.data, item)
	outpath = os.path.join(PATHS.local, item) 
	...
	#                  "Sets: {}").format(arr2str(set2num["IC86_2011"])),
	    "IC86_2016": ("IDs for MC for PS version-004-p00 IC86 2016. " +
	                       "Sets: {}").format(arr2str(set2num["IC86_2016"]))
	}
