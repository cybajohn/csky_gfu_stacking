...
"""
Make complete healpy logLLH maps from EHE scans and save in npy or json as
simple arrays.

Find original EHE rescan files at:
    /data/ana/EHE/IC86_EHE/EHEAlertHistoricalEvents/MillipedeScan/EHE_Scan_final
...
ehe_inpath = os.path.join("/data", "ana", "EHE", "IC86_EHE",
        "EHEAlertHistoricalEvents", "MillipedeScan", "EHE_scan_final",
        "candidate_201[012345]_EHE_alert_?.i3_event0000")
folders = sorted(glob(ehe_inpath))
print("Found {} scan folders:".format(len(folders)))
for folder in folders:
    print("  {}".format(folder))

outpath = os.path.join(PATHS.data, "ehe_scan_maps")
...
outpath = os.path.join(PATHS.local, "ehe_scan_maps_truncated")
if not os.path.isdir(outpath):
    os.makedirs(outpath)
...
hese_inpath = os.path.join("/data", "ana", "IC79", "starting-event",
                      "IC170922A_scans_all_historrical_events",
                      "??????.i3.bz2_event0000")

folders = sorted(glob(hese_inpath))
print("Found {} scan folders:".format(len(folders)))
for folder in folders:
    print("  {}".format(folder))
